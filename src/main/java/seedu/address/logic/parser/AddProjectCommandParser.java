...
public class AddProjectCommandParser implements Parser<AddProjectCommand> {
    ...
    public AddProjectCommand parse(String args) throws ParseException {
        ArgumentMultimap argMultimap =
                ArgumentTokenizer.tokenize(args, PREFIX_PROJECT_NAME, PREFIX_AUTHOR, PREFIX_PROJECT_TAG);

        if (!arePrefixesPresent(argMultimap, PREFIX_PROJECT_NAME, PREFIX_AUTHOR, PREFIX_PROJECT_TAG)
                || !argMultimap.getPreamble().isEmpty()) {
            throw new ParseException(String.format(MESSAGE_INVALID_COMMAND_FORMAT, AddProjectCommand.MESSAGE_USAGE));
        }

        ProjectName pName = ParserUtil.parseProjectName(argMultimap.getValue(PREFIX_PROJECT_NAME).get());
