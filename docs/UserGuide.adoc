...
=== Edit your profile picture : `profilepic`

Changes your profile pic to a certain image.

Format: `profilepic [PATH]`

* PATH is a relative link from the current application location (OASIS.jar) to the image file.
* PATH must link to an image.
* If PATH is not specified, removes your profile pic
